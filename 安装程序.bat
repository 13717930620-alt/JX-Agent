...
echo  ╔═══════════════════════════════════════════════════════════╗
echo  ║                                                         ║
echo  ║     🧠 HyperAgent v5.0 Ultimate — 超级智能体            ║
echo  ║     安装程序                                            ║
echo  ║                                                         ║
...
:: 获取安装目录
:: ============================================
set "DEFAULT_DIR=%USERPROFILE%\HyperAgent"
...
if not exist "!INSTALL_DIR!\.env" (
    ...
    :: 写入 .env 配置
    (
        echo # HyperAgent v5.0 Ultimate 环境变量配置
        echo # ============================================
...
echo @echo off
echo chcp 65001 ^>nul
echo title HyperAgent v5.0 Ultimate
echo cd /d "!INSTALL_DIR!"
echo.
echo cls
echo echo.
echo echo  ╔══════════════════════════════════════════════════╗
echo echo  ║     🧠 HyperAgent v5.0 Ultimate                ║
echo echo  ║     超级智能体 — 正在启动...                    ║
echo echo  ╚══════════════════════════════════════════════════╝
echo echo.
echo node HyperAgent_Main.js server
echo pause
) > "!STARTUP_SCRIPT!"
...
echo @echo off
echo chcp 65001 ^>nul
echo title HyperAgent v5.0 CLI
echo cd /d "!INSTALL_DIR!"
echo cls
echo echo.
echo echo  ╔══════════════════════════════════════════════════╗
...
echo echo  ╚══════════════════════════════════════════════════╝
echo echo.
echo node HyperAgent_Main.js interactive
) > "!CLI_SCRIPT!"

:: 桌面快捷方式
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%USERPROFILE%\Desktop\HyperAgent网页版.lnk'); $s.TargetPath = '!STARTUP_SCRIPT!'; $s.WorkingDirectory = '!INSTALL_DIR!'; $s.Description = 'HyperAgent v5.0 Ultimate - Web界面'; $s.IconLocation = '%SystemRoot%\System32\shell32.dll,165'; $s.Save()" 2>nul

powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%USERPROFILE%\Desktop\HyperAgent命令行.lnk'); $s.TargetPath = '!CLI_SCRIPT!'; $s.WorkingDirectory = '!INSTALL_DIR!'; $s.Description = 'HyperAgent v5.0 Ultimate - CLI交互'; $s.IconLocation = '%SystemRoot%\System32\shell32.dll,166'; $s.Save()" 2>nul
...
echo ════════════════════════════════════════════════════════════
echo.
echo  ✅ HyperAgent v5.0 Ultimate 安装成功！
echo.
echo  📁 安装目录: !INSTALL_DIR!
echo.
echo  🚀 启动方式：
echo  ─────────────────────────────────────────────────────
echo  方式1: 双击桌面 "HyperAgent网页版" 快捷方式
echo  方式2: 双击桌面 "HyperAgent命令行" 快捷方式
...
echo.
echo 感谢使用 HyperAgent！按任意键退出安装程序...
pause >nul
