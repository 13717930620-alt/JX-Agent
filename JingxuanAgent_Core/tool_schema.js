 ...
 *
 * 将 JingxuanAgent 内部工具定义转换为 LLM 函数调用格式 (OpenAI/DeepSeek 兼容)
 * 以及反向转换。
 */
...
/**
 * [toCapabilityMap] 将 LLM 调用的 toolCalls 结果映射回 JingxuanAgent 动作
 * @param {Array} toolCalls - LLM 返回的 tool_calls 数组
 * @returns {Array} JingxuanAgent 格式的动作列表 [{ action, params }]
 */
