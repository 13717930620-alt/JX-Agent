...
const { ToolRegistry, ToolDefinition, ToolUseContext, createBuiltinTools, Schema }
  = require('./HyperAgent_Core/cc_mode/HyperAgent_CC_ToolSystem.js');
const CCContextManager = require('./HyperAgent_Core/cc_mode/HyperAgent_CC_ContextManager.js');
const CCQueryEngine = require('./HyperAgent_Core/cc_mode/HyperAgent_CC_QueryEngine.js');
const PluginLoader = require('./HyperAgent_PluginAPI.js');
const SelfLearning = require('./HyperAgent_Learning.js');

const { Logger, CircuitBreaker, retry, Metrics, HealthCheck, RateLimiter } = require('./HyperAgent_Core/infra/HyperAgent_Infra.js');
const Storage = require('./HyperAgent_Core/infra/HyperAgent_Storage.js');
const SecuritySandbox = require('./HyperAgent_Core/infra/HyperAgent_Security.js');

const StateManager = require('./HyperAgent_Implementation/state_manager/StateManager');
const MemoryManager = require('./HyperAgent_Implementation/memory_engine/MemoryManager');
const AtomicExecutor = require('./HyperAgent_Implementation/atomic_executor/AtomicExecutor');
const ToolExecutor = require('./HyperAgent_Implementation/atomic_executor/ToolExecutor');
const SOPGenerator = require('./HyperAgent_Implementation/sop_generator/SOPGenerator');
const SOPOptimizer = require('./HyperAgent_Implementation/sop_generator/SOPOptimizer');
const CoordinatorOrchestrator = require('./HyperAgent_Implementation/orchestrator/CoordinatorOrchestrator');
const PluginRegistry = require('./HyperAgent_Core/plugin_system/PluginRegistry');
const ContextManager = require('./HyperAgent_Implementation/context_manager/ContextManager');
const ConversationEngine = require('./HyperAgent_Implementation/conversation/ConversationEngine');
const EpisodicMemoryConsolidator = require('./HyperAgent_Implementation/memory_engine/EpisodicMemoryConsolidator');
const LocalInferenceEngine = require('./HyperAgent_Implementation/memory_engine/LocalInferenceEngine');
const ContinualLearner = require('./HyperAgent_Implementation/memory_engine/ContinualLearner');
const MemoryPipeline = require('./HyperAgent_Implementation/memory_engine/MemoryPipeline');
const WorkRecordManager = require('./HyperAgent_Implementation/memory_engine/WorkRecordManager');
const CognitiveFramework = require('./HyperAgent_Core/cognitive_core/CognitiveFramework');
const CognitiveOrchestrator = require('./HyperAgent_Core/cognitive_core/CognitiveOrchestrator');
const DeviceManager = require('./HyperAgent_Implementation/device_abstraction/DeviceManager');
const { PermissionSystem } = require('./HyperAgent_Implementation/permission/PermissionSystem');
const TunnelService = require('./services/TunnelService');
const ConfigManager = require('./services/ConfigManager');

let ExecutionLogger, MetricsCollector, ModelRouter, ModelFallbackChain;
try { ExecutionLogger = require('./HyperAgent_Monitoring/ExecutionLogger'); } catch (e) {}
try { MetricsCollector = require('./HyperAgent_Monitoring/MetricsCollector'); } catch (e) {}
try {
    ModelRouter = require('./HyperAgent_Core/llm_adapter/ModelRouter');
    ModelFallbackChain = require('./HyperAgent_Core/llm_adapter/ModelFallbackChain');
} catch (e) {}

const HyperAgentUpgradeIntegrator = require('./HyperAgent_Implementation/orchestrator/HyperAgentUpgradeIntegrator');

class HyperAgent {
    constructor() {
        this.version = '5.2.0';
        this.name = 'HyperAgent';
        this.ready = false;
        this.components = {};
        ...
        this.log = new Logger('HyperAgent', process.env.LOG_LEVEL || 'info');
    ...
    _validateConfig() {
        try {
            const config = require('./HyperAgent_Config.js');
            const adapter = (config.llm || {}).adapter || 'mock';
            if (adapter !== 'mock') {
                const provider = config.llm[adapter];
                if (!provider || !provider.apiKey) {
                    console.warn(`[HyperAgent] "${adapter}" selected but no API key set`);
                }
            }
        } catch (e) {}
    }
    ...
    async init() {
        ...
        try {
            ...
            try { config = require('./HyperAgent_Config.js'); } catch (e) {}
...
- 被要求"找问题、修问题"时按: 诊断 → 分析 → 修复 → 验证 流程执行`);
            console.log(`[HyperAgent] CC模式: ${this.toolRegistry.getEnabledTools().length} tools ready`);

            this.pluginLoader = new PluginLoader(this.toolRegistry);
            const pluginResult = await this.pluginLoader.discover();
            if (pluginResult.loaded.length > 0) {
                console.log(`[HyperAgent] 插件: ${pluginResult.loaded.length} 加载, ${pluginResult.failed.length} 失败`);
            }
        ...
        this.log.info('SelfLearning ready');
            ...
            console.log(`[HyperAgent] MemoryPipeline READY`);
            ...
            if (resumeText) {
                ...
                console.log(`[HyperAgent] 已加载工作记录恢复上下文`);
            }
            ...
            try {
                ...
                console.warn('[HyperAgent] Cross-session memory load failed:', e.message);
            }
            ...
            try {
                const cfg = require('./HyperAgent_Config.js');
            ...
            try {
                ...
                if (cfConfig.enabled !== false) {
                    this.components.cognitiveFramework = new CognitiveFramework({
                        ...
                        name: deviceConfig.name || `HyperAgent-${deviceConfig.type}`,
                    ...
                    console.log(`[HyperAgent] CognitiveFramework READY (stage: ${this.components.cognitiveFramework.getStatus().cognition.stage})`);
                }
            } catch (e) {
                console.warn('[HyperAgent] CognitiveFramework init failed:', e.message);
            }
            ...
            try {
                if (this.components.cognitiveFramework) {
                    ...
                    console.log('[HyperAgent] CognitiveOrchestrator READY');
                }
            } catch (e) {
                console.warn('[HyperAgent] CognitiveOrchestrator init failed:', e.message);
            }
            ...
            try {
                ...
                console.log('[HyperAgent] Productivity Core READY');
            } catch (e) {
                console.error('[HyperAgent] Productivity Core integration failed:', e);
            }
            ...
            try {
                ...
                if (liConfig.enabled !== false) {
                    ...
                    console.log(`[HyperAgent] LocalInference ${liReady ? 'READY' : 'DISABLED'} (${liConfig.model || 'none'})`);
                        ...
                        if (embedMode === 'local' || embedMode === 'hybrid') {
                            setImmediate(() => {
                                this.components.memoryPipeline.vectorStore.buildEmbeddings({ force: false })
                                    .then(r => console.log(`[HyperAgent] Local embeddings: ${r.indexed}/${r.total}`))
                                    .catch(() => {});
                            });
                        }
                    }
                ...
                console.warn('[HyperAgent] LocalInference init failed:', e.message);
            }
            ...
            try {
                ...
                console.warn('[HyperAgent] ContinualLearner init failed:', e.message);
            }
            ...
            try {
                ...
                if (mcpPlugin && !this.components.mcpBridge) {
                    ...
                    console.log('[HyperAgent] MCP Bridge plugin tracked in components');
                }
            } catch (e) {}
            ...
            try {
                ...
                this.components.upgrade = await HyperAgentUpgradeIntegrator.integrate(
                ...
                console.warn('[HyperAgent] v4.0 upgrade integration warning (non-fatal):', e.message);
            }

            this.health.runAll().then(h => this.log.info('Health check', { status: h.status })).catch(() => {});

            console.log(`[HyperAgent] v${this.version} 启动完成`);
            console.log(`[HyperAgent] 模块: 记忆系统=${!!this.components.memoryManager} 编排器=${!!this.components.orchestrator} 执行器=${!!this.components.atomicExecutor}`);
            console.log(`[HyperAgent] 工具: ${this.toolRegistry?.getEnabledTools().length || 0} 个已注册`);
            console.log(`[HyperAgent] 配置: ${this._configManager?.getConfigSource() || '内置'}`);
            this.ready = true;
            return {
                success: true,
            ...
            console.error('[HyperAgent] Init failed:', e);
    ...
    _registerBuiltinDevices() {
        try {
            const AD = require('./HyperAgent_Implementation/device_abstraction/AutomotiveDevice');
    ...
    _initOptionalComponents() {
        if (ModelRouter && ModelFallbackChain) {
            try {
                ...
                console.log(`[HyperAgent] ModelRouter: ${this.components.modelRouter.models.size} models registered`);
            } catch (e) {
                console.warn('[HyperAgent] ModelRouter init failed:', e.message);
            }
        }
        ...
        try {
            ...
            if (config.mcpServers && Object.keys(config.mcpServers).length > 0) {
                ...
                console.log('[HyperAgent] Computer control capability loaded (inactive, requires user permission)');
            }
        } catch (e) {}
    }
    ...
    async enableRemoteAccess(port) {
        ...
        try {
            ...
            console.error('[HyperAgent] Remote access failed:', e.message);
            return { success: false, error: e.message };
        }
    }
    ...
    async _applyConfigChange(intent) {
        ...
        switch (intent.action) {
            ...
            case 'set': {
                ...
                return { notification: `✅ ${intent.key} 已更新为 ${intent.value}。${cm.requiresRestart(intent.key) ? '需要重启 HyperAgent 才能生效。' : '已即时生效。'}` };
            }
            case 'toggle': {
            ...
            case 'reset': {
                await cm.reset();
                return { notification: '✅ 配置已重置为默认值。请重启 HyperAgent 生效。' };
            }
    ...
    async _hotSwapLLM() {
        delete require.cache[require.resolve('./HyperAgent_Config.js')];
        ...
        console.log('[HyperAgent] LLM adapter hot-swapped');
        return true;
    }
    ...
    _initModelAdapter(modelCfg) {
        ...
        switch (adapterType) {
            case 'glm':
                if (config?.apiKey) {
                    const GLMAdapter = require('./HyperAgent_Core/llm_adapter/GLMAdapter');
                    return new GLMAdapter(config);
                }
                break;
            case 'minimax':
                if (config?.apiKey) {
                    const MinimaxAdapter = require('./HyperAgent_Core/llm_adapter/MinimaxAdapter');
            ...
            case 'deepseek':
                if (config?.apiKey) {
                    if ((config.baseUrl || '').includes('/anthropic')) {
                        const AnthropicCompatAdapter = require('./HyperAgent_Core/llm_adapter/AnthropicCompatAdapter');
                        return new AnthropicCompatAdapter(config);
                    }
                    const DeepSeekAdapter = require('./HyperAgent_Core/llm_adapter/DeepSeekAdapter');
                    return new DeepSeekAdapter(config);
                }
                break;
            case 'qwen':
                if (config?.apiKey) {
                    const QwenAdapter = require('./HyperAgent_Core/llm_adapter/QwenAdapter');
    ...
    async _initLLM() {
        ...
        try {
            const userConfig = require('./HyperAgent_Config.js');
            if (userConfig.llm) config = userConfig.llm;
        } catch (e) {}

        for (const [adapterName, check] of Object.entries({
            glm: (c) => c.glm?.apiKey ? { Adapter: require('./HyperAgent_Core/llm_adapter/GLMAdapter'), cfg: { apiKey: c.glm.apiKey, baseUrl: c.glm.baseUrl, model: c.glm.model, maxTokens: c.glm.maxTokens, temperature: c.glm.temperature } } : null,
            minimax: (c) => c.minimax?.apiKey ? { Adapter: require('./HyperAgent_Core/llm_adapter/MinimaxAdapter'), cfg: { apiKey: c.minimax.apiKey, baseUrl: c.minimax.baseUrl, model: c.minimax.model } } : null,
            deepseek: (c) => c.deepseek?.apiKey ? (c.deepseek.baseUrl||'').includes('/anthropic')
                ? { Adapter: require('./HyperAgent_Core/llm_adapter/AnthropicCompatAdapter'), cfg: { apiKey: c.deepseek.apiKey, baseUrl: c.deepseek.baseUrl, model: c.deepseek.model, maxTokens: c.deepseek.maxTokens, temperature: c.deepseek.temperature } }
                : { Adapter: require('./HyperAgent_Core/llm_adapter/DeepSeekAdapter'), cfg: { apiKey: c.deepseek.apiKey, baseUrl: c.deepseek.baseUrl, model: c.deepseek.model } } : null,
            qwen: (c) => c.qwen?.apiKey ? { Adapter: require('./HyperAgent_Core/llm_adapter/QwenAdapter'), cfg: { apiKey: c.qwen.apiKey, baseUrl: c.qwen.baseUrl, model: c.qwen.model } } : null,
        })) {
        ...
        if (config.adapter !== 'mock') {
            console.warn(`[HyperAgent] ${config.adapter} selected but ${config[config.adapter]?.apiKey ? 'failed to load' : 'no API key set'}`);
        }
        return {
    ...
    async chat(userMessage, options = {}) {
        if (!this.ready) throw new Error('HyperAgent not initialized');
    ...
    async runParallel(tasks) {
        const ParallelPipeline = require('./HyperAgent_Implementation/orchestrator/ParallelPipeline');
...
process.on('uncaughtException', (error) => {
    console.error('[HyperAgent] Uncaught exception:', error);
    if (process.env.NODE_ENV === 'production') process.exit(1);
});
process.on('unhandledRejection', (reason) => {
    console.error('[HyperAgent] Unhandled rejection:', reason);
});

process.on('SIGINT', async () => {
    console.log('\n[HyperAgent] 收到关闭信号，正在清理...');
    ...
    if (global.__hyperAgentInstance?.components?.workRecord) {
        try {
            ...
            console.log('[HyperAgent] 工作记录已保存');
        } catch (e) {
            console.warn('[HyperAgent] 工作记录保存失败:', e.message);
        }
    }
    console.log('[HyperAgent] 清理完成，正在退出...');
    process.exit(0);
});
...
async function main() {
    ...
    if (!initResult.success) {
        console.error('[HyperAgent] Init failed:', initResult.error);
        process.exit(1);
    }
